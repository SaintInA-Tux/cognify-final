...
const Select = ({ value, onChange, options }: { value: string; onChange: (v: string) => void; options: { value: string; label: string }[] }) => (
  <select
    value={value}
    onChange={e => onChange(e.target.value)}
    style={{ width: '100%', padding: '10px 14px', background: 'var(--s2)', border: '1px solid var(--bdr)', borderRadius: 10, color: value ? 'var(--thi)' : 'var(--tdm)', fontFamily: "'Jost', sans-serif", fontSize: 13, outline: 'none', appearance: 'none' }}
  >
...
const Toggle = ({ label, sub, value, onChange }: { label: string; sub?: string; value: boolean; onChange: (v: boolean) => void }) => (
  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 0', borderBottom: '1px solid var(--bdr)' }}>
    ...
    <div
      ...
      style={{ width: 40, height: 22, borderRadius: 11, background: value ? 'var(--thi)' : 'var(--s5)', border: '1px solid var(--bdrhi)', position: 'relative', transition: 'background .2s', flexShrink: 0 }}
    >
...
export default function SettingsPage() {
  ...
  const loadProfile = async () => {
    try {
      ...
      setTargetYear(p.target_year || '2026');
      ...
      if (authProfile) {
        ...
        setTargetYear(authProfile.target_year || '2026');
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await updateProfile({ name, level, exam_board: examBoard, target_exam: targetExam, target_year: targetYear, daily_goal: dailyGoal });
      setSaved(true);
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)', color: 'var(--thi)', fontFamily: "'Jost', sans-serif", display: 'flex', flexDirection: 'column' }}>
      ...
      <nav style={{ height: 50, background: 'rgba(10,10,10,.97)', backdropFilter: 'blur(12px)', borderBottom: '1px solid var(--bdr)', display: 'flex', alignItems: 'center', padding: '0 20px', gap: 12, flexShrink: 0, zIndex: 10, position: 'sticky', top: 0 }}>
        <button onClick={() => navigate(-1)} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, color: 'var(--tlo)', background: 'none', border: '1px solid var(--bdr)', borderRadius: 20, padding: '5px 12px', fontFamily: "'Jost', sans-serif", transition: 'all .2s', textDecoration: 'none' }}>← Back</button>
        ...
        <button
          onClick={handleSave}
          disabled={saving}
          style={{ background: saved ? 'var(--s3)' : 'var(--thi)', color: saved ? 'var(--tmd)' : 'var(--bg)', fontFamily: "'Jost', sans-serif", fontSize: 12, fontWeight: 700, letterSpacing: '.06em', padding: '7px 18px', borderRadius: 8, border: 'none', transition: 'all .2s', display: 'flex', alignItems: 'center', gap: 6 }}
        >
        ...
        <div style={{ position: 'sticky', top: 56, display: 'flex', flexDirection: 'column', gap: 0 }}>
          ...
          {sections.map(s => (
            <a
              ...
              style={{ display: 'flex', alignItems: 'center', gap: 0, height: 44, padding: '0 4px', borderRadius: 12, textDecoration: 'none', border: '1px solid transparent', background: active === s.id ? 'transparent' : 'transparent', transition: 'background .2s', marginBottom: 2 }}
            >
          ...
          <div style={{ marginTop: 24, paddingTop: 16, borderTop: '1px solid var(--bdr)' }}>
            <button
              ...
              style={{ width: '100%', padding: '10px 12px', background: 'transparent', border: '1px solid rgba(208,80,64,.2)', borderRadius: 10, color: 'var(--danger)', fontFamily: "'Jost', sans-serif", fontSize: 12.5, transition: 'all .2s', textAlign: 'left' as const }}
            >
              🚪 Sign out
            </button>
          </div>
              ...
              <div style={{ background: 'var(--s1)', border: '1px solid var(--bdrhi)', borderRadius: 16, padding: '24px', position: 'relative', overflow: 'hidden' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 120px', gap: 16 }}>
                  ...
                  <Field label="Target Year">
                    <Input value={targetYear} onChange={setTargetYear} placeholder="2026" />
                  </Field>
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <div style={{ background: 'var(--s1)', border: '1px solid var(--bdrhi)', borderRadius: 16, padding: '24px', position: 'relative', overflow: 'hidden', marginBottom: 16 }}>
                ...
                <input
                  ...
                  style={{ width: '100%', height: 4, borderRadius: 2, background: 'var(--s5)', outline: 'none', cursor: 'pointer', appearance: 'none', WebkitAppearance: 'none', marginTop: 12 }}
                />
          ...
          {active === 'practice' && (
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                <button onClick={() => navigate('/dashboard')} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '16px 18px', background: 'var(--s1)', border: '1px solid var(--bdrhi)', borderRadius: 14, transition: 'all .2s', textAlign: 'left' as const }}>
                ...
                <button onClick={() => navigate('/challenge')} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '16px 18px', background: 'var(--s1)', border: '1px solid var(--bdrhi)', borderRadius: 14, transition: 'all .2s', textAlign: 'left' as const }}>
                  <span style={{ fontSize: 22 }}>⚡</span>
                  <div>
